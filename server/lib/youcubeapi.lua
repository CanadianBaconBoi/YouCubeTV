...
local function reset_monitor(monitor)
    for i = 0, 15 do
...
local function play_vid_monitor(buffer, monitor, string_unpack, control_table)
    if not string_unpack then
        string_unpack = string.unpack
    end
    local Fwidth, Fheight = monitor.getSize()
    local tracker = 0
    ...
    while true do
        ...
        for y = 1, height do
            ...
            monitor.blit(text[y], fg, bg)
        end
        pos = pos - 2
        local r, g, b
        for i = 0, 15 do
            r, g, b, pos = string_unpack("BBB", data, pos)
            monitor.setPaletteColor(2 ^ i, r / 255, g / 255, b / 255)
        end
        if fps == 0 then
            read()
            break
...
return {
    ...
    play_vid_monitor     = play_vid_monitor,
